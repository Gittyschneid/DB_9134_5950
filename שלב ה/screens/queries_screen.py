...
class QueriesScreen(tk.Frame):
    def __init__(self, parent, app):
        ...
        # ── Controls ──
        controls = Card(self)
        controls.pack(fill="x", padx=PAD_L, pady=(PAD_L, PAD_M), ipadx=PAD_M, ipady=PAD_M)
        ...
        StyledButton(
            btn_row, text="Monthly Workload Report",
            command=self.run_monthly_workload,
            variant="primary", width=28,
        ).grid(row=0, column=0, padx=(0, PAD_S), pady=PAD_XS)

        StyledButton(
            btn_row, text="Understaffed Departments",
            command=self.run_understaffed_depts,
            variant="primary", width=28,
        ).grid(row=0, column=1, padx=(PAD_S, 0), pady=PAD_XS)
    ...
    def run_monthly_workload(self):
        self.desc_label.config(text="Monthly Workload Report — shifts per staff member per month.")
        query = """
            SELECT
    ...
    def run_understaffed_depts(self):
        self.desc_label.config(text="Departments with fewer than 30 nurses — staffing analysis.")
        query = """
            SELECT
                d.department_name,
                d.location,
